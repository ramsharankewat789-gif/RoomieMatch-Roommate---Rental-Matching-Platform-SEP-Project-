...
async function listProperties(req, res) {
  try {
    ...
    const { search, city, type, minPrice, maxPrice, bedrooms, status, verified, ownerId } = req.query;
    ...
    if (search) {
      conditions.push("(p.title LIKE ? OR p.address LIKE ? OR p.city LIKE ? OR p.description LIKE ?)");
      const s = `%${search}%`;
      params.push(s, s, s, s);
    }
    ...
    if (status && isAdmin)   { conditions.push("p.status = ?");      params.push(status); }
    ...
    const [countRow] = await all(
      `SELECT COUNT(*) AS total FROM properties p ${where}`, params
    );
...
async function getProperty(req, res) {
  try {
    ...
    const isAdmin = req.user?.role === "admin";
    if (!isOwner && !isAdmin && (prop.status !== "active" || !prop.is_verified)) {
...
async function updateProperty(req, res) {
  try {
    ...
    const existing = await get("SELECT id, owner_id FROM properties WHERE id = ?", [id]);
    ...
    if (available_from !== undefined) { updates.push("available_from = ?"); params.push(available_from || null); }

    if (updates.length > 0) {
      params.push(id);
      await run(`UPDATE properties SET ${updates.join(", ")} WHERE id = ?`, params);
    }
    ...
    if (Array.isArray(rules)) {
      ...
      for (const r of rules) {
        ...
        if (trimmed) {
          await run("INSERT INTO property_rules (property_id, rule) VALUES (?, ?)", [id, trimmed]);
        }
      }
    }
