    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      <section className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm flex flex-col md:flex-row justify-between items-start gap-4">
        <div className="space-y-2">
          ...
          <h1 className="font-headline-md text-headline-md text-on-surface font-bold">{property.title}</h1>
        ...
        <div className="flex flex-row md:flex-col items-end gap-3 justify-between w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0">
          ...
          >
            <span className={`material-symbols-outlined text-[20px] ${isFavourited ? "text-error icon-fill" : ""}`}>
              favorite
            </span>
      ...
      {/* Cover image */}
      <section className="rounded-xl overflow-hidden shadow-sm border border-outline-variant max-h-[420px] bg-surface-container">
        {coverImage ? (
          <img src={coverImage} alt={property.title} className="w-full h-full object-cover max-h-[420px]" />
        ) : (
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {property.images.map((img, i) => (
            <img
              ...
              className="w-28 h-20 object-cover rounded-lg border border-outline-variant shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
            />
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-1 mt-0.5">
                  {(property.owner?.is_verified || property.owner?.isVerified) ? (
                    ...
                    <span className="text-xs text-outline font-semibold">Member</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {!isOwnProperty && (
                <>
                  ...
                  <Button
                    variant="primary"
                    onClick={() => { setApplyOpen(true); setAppError(""); }}
          ...
          <section className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm">
            <h3 className="font-headline-sm text-headline-sm text-on-surface mb-4">Details</h3>
            <div className="space-y-3 font-body-md text-body-md text-on-surface-variant">
              ...
              <div className="flex justify-between border-b border-outline-variant/60 pb-2">
                <span>Type</span>
                <span className="font-bold text-on-surface">{property.type}</span>
              </div>
              <div className="flex justify-between pb-1">
                <span>Status</span>
                <span className="font-bold text-on-surface capitalize">{property.status}</span>
              </div>
            </div>
          </section>
        </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setApplyOpen(false)} disabled={appLoading}>
              Cancel
            </Button>
            <Button variant="primary" onClick={handleApplySubmit} disabled={appLoading || !appMessage.trim()}>
