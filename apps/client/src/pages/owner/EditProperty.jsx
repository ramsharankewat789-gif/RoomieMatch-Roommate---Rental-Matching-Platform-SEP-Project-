...
export const EditProperty = () => {
  ...
  useEffect(() => {
    if (property) {
      setTitle(property.title); setAddress(property.address); setCity(property.city);
      setType(property.type); setBedrooms(String(property.bedrooms)); setBathrooms(String(property.bathrooms));
      setPrice(String(property.price)); setDeposit(String(property.deposit));
      setDescription(property.description); setRulesStr(property.rules?.join(", ") || "");
  ...
  useEffect(() => {
    ...
    apiGetPropertyImages(id)
      ...
      .catch(() => {
        ...
        if (property?.images) {
          setServerImages(property.images.map((url, i) => ({ id: `local_${i}`, image_path: url, is_primary: i === 0 })));
        }
      });
  }, [id]);
  ...
  if (!property) return (
    <div className="max-w-xl mx-auto py-12 text-center bg-surface-container-lowest border rounded-xl">
      ...
      <h3 className="font-headline-sm text-on-surface font-bold mt-2">Listing Not Found</h3>
      <Button onClick={() => navigate("/user/my-properties")} className="mt-4 mx-auto">Back to Listings</Button>
    </div>
  );
  ...
  const uploadPending = async () => {
    ...
    setImageError("");
    try {
      const data = await apiUploadPropertyImages(id, pendingFiles.map((p) => p.file));
  ...
  const handleSetPrimary = async (imgId) => {
    if (imgId.startsWith("local_")) {
      setServerImages((prev) => prev.map((img) => ({ ...img, is_primary: img.id === imgId ? 1 : 0 })));
      return;
    }
    setImgLoading(true);
    try {
  ...
  const typeOptions   = [{ value: "Apartment", label: "Apartment" },{ value: "Townhouse", label: "Townhouse" },{ value: "Studio", label: "Studio" },{ value: "House", label: "Single House" }];
  ...
  return (
    <div className="max-w-3xl mx-auto bg-surface-container-lowest p-8 rounded-xl border border-outline-variant shadow-sm">
      <div className="mb-6">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant mt-0.5">Modify details, pricing, rules, and photos</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input label="Property Title" value={title} onChange={(e) => setTitle(e.target.value)} required />
          <Select label="Property Type" value={type} onChange={(e) => setType(e.target.value)} options={typeOptions} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Input label="Address" value={address} onChange={(e) => setAddress(e.target.value)} required containerClassName="md:col-span-2" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Input label="Rent ($/month)" type="number" value={price} onChange={(e) => setPrice(e.target.value)} required />
          <Input label="Deposit ($)" type="number" value={deposit} onChange={(e) => setDeposit(e.target.value)} />
        </div>

        <Textarea label="Detailed Description" value={description} onChange={(e) => setDescription(e.target.value)} rows={5} required />
        <Input label="House Rules (comma-separated)" value={rulesStr} onChange={(e) => setRulesStr(e.target.value)} />
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="font-label-md text-label-md text-on-surface-variant font-bold">Property Photos</span>
            <span className="text-xs text-outline">{totalImages}/{MAX_IMAGES}</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {serverImages.map((img) => (
              <div key={img.id} className="relative group">
                <img src={img.image_path} alt="" className={`w-24 h-20 object-cover rounded-xl border-2 transition-all ${img.is_primary ? "border-primary shadow-md" : "border-outline-variant"}`} />
                {img.is_primary === 1 && (
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-1">
                  {!img.is_primary && (
                    <button type="button" onClick={() => handleSetPrimary(img.id)} className="bg-primary text-on-primary rounded-full p-1">
                      <span className="material-symbols-outlined text-[14px]">star</span>
                    </button>
                  )}
                  <button type="button" onClick={() => handleDeleteServerImage(img.id)} className="bg-error text-on-error rounded-full p-1">
            ...
            {pendingFiles.map((p, idx) => (
              <div key={idx} className="relative group">
                <img src={p.preview} alt="" className="w-24 h-20 object-cover rounded-xl border-2 border-dashed border-primary/60 opacity-80" />
                <span className="absolute top-1 left-1 bg-secondary text-on-secondary text-[9px] font-bold px-1.5 py-0.5 rounded-full">New</span>
                <button type="button" onClick={() => { URL.revokeObjectURL(p.preview); setPendingFiles((prev) => prev.filter((_, i) => i !== idx)); }}
          ...
          {imgLoading && (
            <p className="text-xs text-on-surface-variant flex items-center gap-1">
              <span className="material-symbols-outlined text-[14px] animate-spin">progress_activity</span>Processing images...
            </p>
          )}
          {imageError && (
          ...
          <p className="text-xs text-outline">JPG, PNG, WEBP · Max {MAX_SIZE_MB} MB · Hover to set primary or delete</p>
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-outline-variant/60">
          ...
          <Button type="submit" variant="primary" disabled={imgLoading}>
            {imgLoading ? (
              <span className="flex items-center gap-2"><span className="material-symbols-outlined text-[16px] animate-spin">progress_activity</span>Saving...</span>
            ) : "Save Changes"}
          </Button>
        </div>
      </form>
